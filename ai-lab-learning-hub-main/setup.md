...
*Note: There is also a screen recording of the below steps [available here](https://microsoftgenmab-my.sharepoint.com/:v:/g/personal/kmar_genmab_com/EUFJ0nn-vb5AuoAUYB12LxcBL32sEVuD7lcCKYl02PbDRw?e=x9cfYJ).*
- **Network Requirement:**
...
- **Accessing the Environment:**
  ...
  - *Important: To help conserve compute resources for all EAP users, please close any unused sessions once you’re finished using them.* You can do this by visiting the [main QA EAP page](https://workbench.qa.eap.genmab.net/) and clicking “Quit” on sessions that are complete. Note that your files and virtual environment setup are retained across sessions, so when you return later you can safely start a new session without losing any content.
...
- **Importing the Course Repository:**
  ...
  - All other roles (e.g. Reporter):
     - Download the full repository from [GitLab](https://gitlab.com/genmab/production/non-gxp/ai-labs/ai-lab-learning-hub/) (select **Code > Download Source Code > zip**).
  - Unzip the file.
  - Drag and drop the entire repository contents into the Explorer bar of your EAP VS Code session. If you did not rename the file then the path to the repository contents should be `4gram/ai-lab-learning-hub-main` where "4gram" is your unique user key.
...
1. **Open a Terminal Session in VS Code:**
   ...
   - Navigate to the home directory of the repository if you are not already there (e.g. `cd ai-lab-learning-hub-main` if that is the name of the repository on your system). You can confirm you are in the right spot if you type `ls` and the results are the repository contents, including the pyproject.toml and uv.lock files.
...
4. **Open Jupyter Notebook in VS Code**:
   - In VS Code, open the Jupyter Notebook file that you wish to run (e.g. Introduction/Introduction_to_API_and_Prompting.ipynb)
   - Click "Select Kernel" in the top-right corner of the notebook interface.
   - If you followed the above steps then we are looking for the kernel associated with your virtual environment ending in the path `.venv/bin/python` (Note: the start of the path may differ depending on the name and location of your repository).
     - In EAP you will select "Python Environments" then look for the environment `ai-lab-learning-hub-main/.venv/bin/python`. It is possible that you may not see it at first, in which case you should follow the steps in the first bullet of the Troubleshooting section below.  
     - For local Python users, if you don't see your kernel immediately then go to `Select Another Kernel` > `Python Environment`, and look there. If you still do not see the kernel, restart VS Code.
   - If you are able to find the kernel and run the first cell of the notebook (with the import lines) then you are fully setup. Congrats!

### Troubleshooting:

- **If you still do not see the kernel at `.venv/bin/python` after step 4**:
  - Navigate to the Command Palette.
     - EAP Users: Click the hamburger bar in the upper left hand side, then select View > Command Palette.
     - Local Python Users: Select View > Command Palette
  - In the Command Palette window type "Python: Select Interpreter" and select it.
  - A new window with "Select Interpreter" at the top will open. Select "Enter Interpreter path" and paste in the full path of the virtual environment, which should be `ai-lab-learning-hub-main/.venv/bin/python` (depending on your repository location, see Note below). Hit Enter.
     - Note: The above path assumes your repository folder is named "ai-lab-learning-hub-main" and the virtual environment was created inside it (you can confirm this is true if there is a folder called `.venv/` within it). This should be the case for all EAP users but if you are a local Python user and you have instead put your repository in another location (e.g. `C:\Users\4gram\Downloads\`) then include the full path before `ai-lab-learning-hub-main`.
  - After you have entered the interpreter path, go back to the notebook and click "Select Kernel" again and look for the kernel at the path (e.g. `ai-lab-learning-hub-main/.venv/bin/python`).
    - If you do not see it on the first screen then choose "Select Another Kernel" > "Python Environments" and you should see it there.
  - Select the kernel and verify that you can run the first cell of the notebook.

- **If you get an "error: Microsoft Visual C++ 14.0 or greater is required..." error after `uv sync`**:
  - This error occurs for some Windows users running Python locally. It should be fixed by the newest committed versions of `pyproject.toml` and `uv.lock` (which were updated to remove the dependencies chromadb and sentencepiece). If you are still seeing it for another package, please reach out.

- *When in doubt, paste your command and error into ChatGPT and ask it for help!*
